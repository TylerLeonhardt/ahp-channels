...
import { inspectPlugin, resolveServerConfig } from '../src/plugins.js';
...
try {
	const channelConfig = resolveServerConfig(plugin);
	...
	const sessionState = await waitForSessionChat(
	...
	console.log(`E2E passed: ${channelInfo.name} received ${reply.trim()} from ${provider}`);
} catch (error) {
	primaryError = error;
} finally {
